            ...
            {missions.map((m) => (
              <div key={m.id} className="relative border-l border-white/5 pl-3 group/block">
                ...
                <div className="flex flex-col gap-1.5 mb-3">
                  {m.targets?.map((target: any) => (
                    <button
                      key={target.word}
                      onClick={() => jumpTo(target.word)}
                      className={`
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className={`w-1 h-1 rounded-full ${target.reviewed ? 'bg-green-600' : 'bg-orange-500 animate-pulse'}`}></div>
                        <span className={`font-mono ${target.reviewed ? 'line-through' : 'font-bold'}`}>{target.word}</span>
                      </div>
                      {target.reviewed && <span className="text-[7px] font-bold">{t(uiLang, 'mission.ok')}</span>}
                    </button>
                  ))}
                </div>
